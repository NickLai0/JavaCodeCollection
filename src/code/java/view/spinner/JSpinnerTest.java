package code.java.view.spinner;
...
public class JSpinnerTest extends JFrame {
    ...
    Box verticalSpinnerBox = new Box(BoxLayout.Y_AXIS);
    ...
    private void setupSouth() {
        ...
        bnPanel.add(okBn);
        add(bnPanel, BorderLayout.SOUTH);
    }
    ...
    private void setupCenter() {
        ...
        SpinnerListModel bookModel = new SpinnerListModel(strArr);
        ...
        add(verticalSpinnerBox, BorderLayout.CENTER);
    }

    // 定义一个方法，用于将滑动条添加到容器中
    public void addSpinner(JSpinner spinner, String description, JLabel valLabel) {
        Box horizontalBox = Box.createHorizontalBox();
        JLabel destLabel = new JLabel(description + "：");
        horizontalBox.add(destLabel);
        horizontalBox.add(spinner);
        horizontalBox.add(valLabel);
        verticalSpinnerBox.add(horizontalBox, BorderLayout.CENTER);

        destLabel.setPreferredSize(new Dimension(100, 30));
        valLabel.setPreferredSize(new Dimension(180, 30));
    }

    public static void main(String[] args) {
        FrameUtils.visibleAndExitOnClose(JSpinnerTest.class)
                .setTitle("微调控制器示范");
    }
